...
/**
 * ContractState is a template-specific JSON structure.
 */
...
/**
 * Agreement represents an APAP agreement instance.
 */
...
/**
 * GeneratedAgreement bundles the rendered agreement text (as Markdown)
...
/**
 * TriggerResult represents the result from triggering a clause.
 * For asynchronous execution, a jobId is returned.
 */
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  try {
    if (uri === "template://list") {
      ...
      return {
        contents: templates.map((t: string) => ({
          uri: `template://${t}`,
          text: t
        }))
      };
    ...
    if (uri === "agreement://list") {
      ...
      return {
        contents: agreements.map((a: string) => ({
          uri: `agreement://${a.replace(/"/g, '')}`,
          text: a
        }))
      };
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "getAgreement") {
    ...
    const { agreementId } = args as { agreementId: string };
  ...
  if (name === "createAgreement") {
    ...
    const { templateId, data, agreementParties, signatures, metadata } = args as { 
      templateId: string; 
      data: unknown;
  ...
  if (name === "trigger") {
    ...
    const { agreementId, request: triggerRequest } = args as { agreementId: string; request: unknown };
